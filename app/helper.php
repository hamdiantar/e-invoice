 ...
 * @param int $limit [the limit of characters of name logo]
 * @return string
 */
function nameLogo(string $name = null, int $limit = 2):string
{
    $words = explode(" ", $name);
    $logo = "";
    for ($i = 0; $i < $limit; $i++) {
 ...
 */
function colors(int $key = null):string
{
 ...
 * @return string
 */
function implodeAsVar(array $array):string
{
    $string = implode("_", array_map('ucfirst', $array));
    return $string;
}
...
function setCheckedInInput(Collection  $model, string $key, bool $withValue = false): ?string
{
    $item = $model->where('key', $key)->first();
   if ($item && $item->value) {
       return $withValue ?  $item->value : "checked";
   }
   return null;
}

function checkStartDayOfWeek(Collection  $model, string $key, string $day): string
{
    $item = $model->where('key', $key)->first();
    if ($item && $item->value === $day) {
        return "checked";
    }
    return "";
}
...
function getModalAttrOrOld($modal){

    return function($key) use($modal){
        return is_null($modal) ? old($key) : $modal[$key];
    };

}
function lang(){
    return app()->getLocale() == "ar"?'_ar':'';
}
function getUserType($slug)
{
    return \App\Models\UserType::select('id')->where('slug',$slug)->first()->id??'';
}
...
{
    return [
        "Monday" => trans('days.Monday'),
        "Tuesday" => trans('days.Tuesday'),
        "Wednesday" => trans('days.Wednesday'),
        "Thursday" => trans('days.Thursday'),
        "Friday" => trans('days.Friday'),
        "Saturday" => trans('days.Saturday'),
        "Sunday" => trans('days.Sunday'),
    ];
}

function hasPermission(string $permission): bool
{
    if (user()->can($permission) || user()->is_admin) {
        return true;
    }
    return false;
}
function generateRandomInvoiceNumber() {
    ...
    } while (Invoice::where("number", "=", $number)->first());
