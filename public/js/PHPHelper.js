...
/**
 * [explode makes php explode "splitting string using delimiter"]
 *
 * @param {string} delimiter [delimiter will be used to explode with it]
 ...
 */
var rtrim = function(string, char, all_after = false)
{
...
/**
 * [singleTrim trims single char from text according to the direction of the trim)]
 *
 * @param {string} char   [character that wanted to be trimmed]
 * @param {string} string [string to trim]
 * @param {string} rol [refer to direction of trim]
 * @return {string} [trimmed text]
 */
var singleTrim = function(string, char, rol = 0)
{
    let string_length = string.length;
    if (!rol){
        let last_index = string.lastIndexOf(char);
        if (last_index == string_length - 1) {
            string = string.slice(0, last_index);
        }
    }else if(rol == 1){
        let index = string.indexOf(char);
        if (index == 0) {
            string = string.slice(1, last_index);
        }
    }else{
        string = singleTrim(string, char, 0);
        string = singleTrim(string, char, 1);
    }
    return string;
}
 ...
 * @param  {string} url       [valid url]
 * @param  {string} type      [valid http method]
...
/**
 * [functionParamBuilder creates function arguments]
 * @param {array} params function original parameter names
 * @param {Object} data function data wanted to be replaced by value into function params
 */
var functionParamBuilder = function(params = [], data)
{
    ...
    for (const index in params) {
        let param = data[params[index]];
        if (!isset(param)) {
            param = data[strtolower(params[index])];
        }
...
function getCurrentOrigin(url = null)
{
    if(!url){
        url = current();
    }
